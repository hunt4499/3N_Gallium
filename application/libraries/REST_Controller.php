...
abstract class REST_Controller extends CI_Controller {
    ...
    protected function early_checks(){
        ...
        if($method == 'get'){
            if(!empty($this->get('v_code'))){
                $secs['v_code'] = $this->get('v_code');
            }
            if(!empty($this->get('apikey'))){
                $secs['apikey'] = $this->get('apikey');
            }
        }
        if($method == 'post'){
            ...
            /////// V Code ////////
            if(!empty($this->req['v_code'])){
                $secs['v_code'] = $this->req['v_code'];
            }
            /////// API Key ////////
            if(!empty($this->req['apikey'])){
                $secs['apikey'] = $this->req['apikey'];
            }
            /////// V Code ////////
            if(!empty($this->input->post('v_code'))){
                $secs['v_code'] = $this->input->post('v_code');
            }
            /////// API Key ////////
            if(!empty($this->input->post('apikey'))){
                $secs['apikey'] = $this->input->post('apikey');
            }
        }

        ///////////////////// Securing By API key And Versions Control //////////////////////

        if(!empty($secs) && isset($secs['v_code']) && isset($secs['apikey'])){
            $this->db->select('*');
            $this->db->from('api_version');
            foreach($secs as $key=>$value){
                $this->db->where($key, $value);    
            }

            $sec_res = $this->db->get()->num_rows();

             if($sec_res != 1){
                $this->response([$this->config->item('rest_status_field_name') => FALSE,
                $this->config->item('rest_message_field_name') => $this->lang->line('text_rest_api_key_unauthorized')], self::HTTP_UNAUTHORIZED);
        ...
        ///////////////////// End  Securing By API key And Versions Control ////////////////
    }
